...
prompt_gen_role = [
    {
        ...
        "content": f"Generate a detailed, creative image using Dall-E, respecting the limit of 4 images per request. Avoid any depictions of politicians, public figures, or specific artists' styles from the last 100 years. Make sure to specify the type of image (e.g., photo, painting, illustration). Do not reference any specific people or celebrities. The description of the image should be concrete, objective, and detailed. The overall prompt length should not exceed {dalle_char_limit} characters.",
...
def get_images(plot_list, art_style):
    ...
    for string in plot_list:
        ...
        prompt = f"A vivid immersive artwork in the {art_style} art style. {summary}"
        print(prompt)
        ...
        #     # model="dalle2",
        # )
        if len(prompt) > (dalle_char_limit + 100):
            prompt = prompt[: (dalle_char_limit + 99)]
